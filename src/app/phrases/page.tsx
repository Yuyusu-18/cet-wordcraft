...
export default async function PhrasesPage({
  ...
  const cet4Count = await prisma.phrase.count({
    where: { status: "published", level: "CET4" },
  });
  const cet6Count = await prisma.phrase.count({
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-gray-800">📖 短语速记</h1>
        <p className="text-gray-500 text-sm mt-1">
          AI 生成的巧记法帮你高效记忆，已发布 {cet4Count + cet6Count} 个短语
        </p>
      </div>
        ...
        {phrases.map((p) => (
          ...
          >
            <div className="flex items-start justify-between mb-1">
              <span className="text-lg font-bold text-gray-800">{p.phrase}</span>
